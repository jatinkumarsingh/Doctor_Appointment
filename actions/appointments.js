...
function normalizeDoctorIdentifier(doctorId) {
  if (!doctorId || typeof doctorId !== "string") {
    return null;
  }

  try {
    return decodeURIComponent(doctorId);
  } catch {
    return doctorId;
  }
}

async function findVerifiedDoctorByIdentifier(doctorId) {
  const normalizedDoctorId = normalizeDoctorIdentifier(doctorId);
  ...
  return db.user.findFirst({
    where: {
      ...
      OR: [{ id: normalizedDoctorId }, { clerkUserId: normalizedDoctorId }],
    },
  });
}
...
export async function bookAppointment(formData) {
  ...
  try {
    ...
    const doctor = await findVerifiedDoctorByIdentifier(doctorId);
...
export async function getAvailableTimeSlots(doctorId) {
  try {
    ...
    // If doctor has not set availability yet, return empty slots for the next 4 days.
    if (!availability) {
      return {
        days: days.map((day) => ({
    ...
    if (
      ...
      error.message.includes("No availability set by doctor")
    ) {
      const now = new Date();
      const days = [now, addDays(now, 1), addDays(now, 2), addDays(now, 3)];
